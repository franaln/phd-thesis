...
El {\SM} de la física de altas energías descripto en la \cref{cap:sm}, con el
agregado de la masa de los neutrinos, y el descubrimiento del bosón de Higgs, ha
...
importantes. El sólo hecho de que la relación $M_P/M_W$ es tan grande es una
gran motivación para la física más allá del {\SM}, por el llamado \emph{problema de jerarquía}.
Esta no es una dificultad intrínseca del SM, sino una preocupante sensibilidad
del potencial de Higgs a nueva física en casi cualquier extensión imaginable del
SM.
...
campo de Higgs. Por ejemplo, en la \cref{fig:higgs_correction_f} se tiene una
...
la correción más grande será cuando $f$ es el quark \emph{top} con
$\lambda_f \approx 1$.
...
\emph{Modelo Estándar Supersimétrico Mínimo}, o MSSM.
...
H_d^-)$. Esta notación tiene como ventaja que es fácil recordar el VEV de que
...
nomenclatura usual para referirse a los supercomaneros de espín $\frac{1}{2}$ es
agregar ``ino'' a la partícula del SM, por lo tanto los compañeros fermiónicos
de los escalares de Higgs son denominados \emph{higgsinos}, y se denotan con
$\susy{H_u}$ y $\susy{H_d}$.
...
supersimétrico de espín $\frac{1}{2}$ es el \emph{gluino}. Los gauginos
supercompañeros de los bosones de gauge electrodebiles, luego de mezclarse con
los supercompañeros de los bosones de Higgs,  dan
lugar a los autoestados de masa denominados \emph{charginos} y
\emph{neutralinos}. En la \cref{tab:sparticles} se puede ver el espectro
completo del MSSM.
...
si esta simetría no esta rota. Aunque no se conoce el mecanismo de ruptura de
...
Para ello las relaciones entre los acoplamientos adimensionales que están en la
teoría antes del rompimiento deben mantenerse. Para que esto suceda, el
rompimiento de la supersimetría debe ser \emph{suave}, y
...
Yukawa y preserva la invarianza supersimétricas.
%% donde $L_{SUSY}$ es invariante bajo transformaciones de gauge de SUSY y
%% $\text{SU}(3)_C \times \text{SU}(2)_L \times \text{U}(1)_Y$, y contiene el
%% lagrangiano MSSM para fermiones y bosones de gauge y el lagrangiano asociado a
%% los sfermiones y gauginos. Tambien incluye todas las interacciones
%% renomalizables entre las particulas supersimetricas y entre estas y las
%% particulas del SM. Tambien contiene el lagrangiano del sector de Higgs, y el de
%% Higgsinos. Este incluye los terminos de autointeraccion del Higgs y las
%% interacciones entre estas particulas y el resto del espectro del MSSM. En
%% particular, incorpora las interacciones de Yukawa entre los bosones de Higgs y los
%% fermiones y el potencial de Higgs. Las restricciones impuestas por SUSY determinan
%% las autointeracciones de los bosones de Higgs en terminos de los acoplamientos
%% de gauge $g$ y $g'$.
El conjunto de parámetros que aparecen en el lagrangiano $\mathcal{L}_\text{SUSY}$ son:

\begin{itemize}\itemsep0.2cm\parskip0.2cm
\item las constantes de acoplamiento de gauge $g_s$, $g$ y $g'$ correspondientes a
  los grupos de gauge $\mathrm{SU}(3)_C$, $\mathrm{SU}(2)_L$ y $\mathrm{U}(1)_Y$, respectivamente,
\item los acoplamientos de Yukawa que describen las interacciones entre fermiones y bosones de Higgs, y
\item el parámetro de masa $\mu$.
\end{itemize}

El lagrangiano que rompe SUSY, $\mathcal{L}_\text{soft}$, no esta completamente
determinado y su forma explícita y el conjunto de parámetros involucrados
dependen del mecanismo particular de ruptura de SUSY. Debido a que este
mecanismo es desconocido, se pueden suponer un conjunto de términos de ruptura
de la forma más general posible, sin indagar en sus origenes, que se fijan solo
pidiendo la invarianza frente $\mathrm{SU}(3)_C \times \mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$,
...
a fin de que sean mas pesadas que sus correspondiente compañeras del SM, y la
...
El conjunto de nuevos parámetros que aparece en el superpotencial de ruptura de SUSY es\cite{PDG,Haber:1993wf}:
...
\item Los acoplamientos trilineares de squarks y sleptones $A_q$ y $A_l$.

\item Las masas \emph{soft} de los gauginos, $M_3$, $M_2$ y $M_1$, asociadas a los
  grupos de gauge del SM $\text{SU}(3)_C$, $\text{SU}(2)_L$ y $\text{U}(1)_Y$, respectivamente.
...
física subyacente que esta asociada con escalas de energía mayores a la de los
experimentos. Pero realizar predicciones y análisis fenomenológicos con esta
...
suposiciones que se han hecho cuando se estudia un determinado análisis.
...
simetría $ \text{SU}(3)_C \times \text{SU}(2)_L \times \text{U}(1)_Y$, se
...
un límite superior en la vida media del protón que es $> 10^{30}$ a\~nos a
$90\%$ CL\cite{PDG}.

Para evitar este problema, la estrategia más comúnmente utilizada es introducir
una nueva simetría. Esta simetría se la conoce como paridad-R y se define
como $P_R = (-1)^{3(B-L)+ 2s}$,
...
%% y la manera usual de
%% solucionar esto es la introduccion de una nueva simetria discrita llamada
%% paridad-R. La paridad-R es un numero cuantico multiplicativo definido en
%% terminos de los numeros leptonico y barionico y el espin, como $P_R =
%% (-1)^{3(B-L)+ 2s}$. Esto implica que las particulas del SM y de Higgs tengan
%% $P_R=1$ y sus supercomaneras SUSY tengan $P_R=-1$.
...
contiene entonces un número par de spartículas, lo cual conlleva a algunas
...
\item Squarks y sleptones
  ...
  para squarks \emph{up}, \emph{down} y sleptones cargados, y una matriz adicional de $3\times
...
\item Sector de Higgs
  ...
  de los bosones de Higgs estan determinadas, a orden arbol y debido a la supersimetría,
  por solo dos parametros que usualmente se elige que sean $m_{A_0}$ y $\tan \beta$.
...
cinemáticamente permitido, porque tiene acoplamiento QCD. Por otro
...
Como se mencionó anteriormente, debido a que no se ha observado ninguna de las
partículas supersimétricas predichas, si existe SUSY, debe estar rota. Para
mantener la solución al problema de jerarquía incluso en presencia del
rompimiento de esta, este rompimiento debe ser suave, incluyendo términos \emph{soft} al
lagrangiano.
...
esperado que sea de al menos el orden de 100 \gev.
%% Sus interacciones van a ser
...
la LSP, y todas las spartículas del MSSM van a decaer eventualmente en un estado
...
de rompimiento de SUSY, la NLSP puede decaer de forma inmediata (\emph{prompt}) o luego de haber
...
evento SUSY se producen dos NLSPs, es claro que los estados finales en un
...
diferentes estados finales en los colisionadores. En principio la NLSP puede ser cualquier particula del
MSSM dependiendo de la elección de los parámetros del modelo. Para la mayor
parte del espacio de parametros será el neutralino ó el stau, y para algunas
regiones muy restrictivas, el sneutrino\cite{arxiv:9801271}. Las partículas, que
aunque no son NLSP, tienen un decaimiento dominante en su compañero
supersimetrico y el {\gravino} se denominan co-NLSP.
...
de $\ninoone \to h\gravino$ donde el Higgs decaerá mayormente como $h\to b \bar{b}$,
dejando un estado final con un fotón, {\bjets} y {\met}. En el escenario donde
$\mu>0$ el decaimiento dominante estará compartido entre $\ninoone \to \gam\gravino$
y $\ninoone\to Z \gravino$, y el decaimiento a Higgs estará suprimido,
