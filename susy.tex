...
El {\SM} de la física de altas energías descripto en el capítulo
...
Si incluimos la gravedad, el gravitón de spin 2 (con dos estados de
...
\begin{figure}[h]
  \centering \includegraphics[width=0.9\textwidth]{figures/sparticles}
  \caption{Caption\note{Rehacer tabla}}
...
En el MSSM, la descripción del rompimiento de la simetría electrodébil
...
Esta matriz de masas puede ser diagonalizada con una matriz unitaria
...
$\lambda_e$. % (correspondientes al acoplamiento de una generación).
...
Para evitar este problema, la estrategia mas comúnmente utilizada es
...
\begin{figure}[!htbp]
  \centering \input{tikz/susy_breaking.tex}
...
Si tenemos en cuenta a la gravedad, SUSY tiene que ser promovida a una
...
Las partículas, que aunque no son NLSP, tienen un decaimiento
...
Los binos decaen a fotones con un BR $\sim \cos^2\theta_W$, con una
...
%% Las BR del bino y wino se pueden ver en la figura XXX, donde vemos que los binos
%% decaen predominantemente a fotones con una BR $\sim \cos^2 \theta_W$, y con
%% una componente a $Z$ con BR $\sim \sin^2 \theta_W$. Por otro lado
%% estos BR se invierten para el caso de que el neutralino sea puramente wino,
%% que decae mayoritariamente a $Z$. Un neutralino puramente higgsino decae dominantemente
%% a $Z$ o $h$, con BR que dependen del valor de $\tan\beta$ y el signo de $\mu$. Tenemos
%% tres casos:
%% (1) el higgsino decae mayormente a $Z$ a bajo $\tan\beta$ y $\mu$ positivo,
%% (2) mayormente a $h$ a bajo $\tan\beta$ y $\mu$ negativo,
%% (3) mezcla de $Z$ y $h$ para moderado o alto $\tan\beta$.

%% Es interesante nota que si la NLSP es casi wino, hay generalemtne una
%% muy pequena diferencia entre el wino cargado y el neutro. Cuando esto
%% ocurre, el decaimiento a tres cuerpos a wino neutro empieza a desaparecer, y
%% el wino cargado prefiere decaer directamente a $W^\pm$ y gravitino. En otras
%% palabras, el wino neutro y el cargado son co-NLSPs, y el estado final contendra $W$,
%% como tambien $Z$ y fotones. Por otro lado la diferencia de masas entre los
%% higgsinos neutros y cargados es generalmente grande, por lo cual solo el neutralino
%% mas liviano decae directamente a gravitino.
...
\note{Explicar que pasa con la mezcla bino-higgsino, que da lugar
  al photon+b y photon+jets? O en la parte de se\~nal?}
...
La producciones en \cref{eq:qq_ewk,eq:qq_ewk2} obtienen contribuciones
...
\hll{Agregar XS para los distintos modos de produccion.}
