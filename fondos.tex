...
Como se describe en el Capítulo (\cref{sec:signal_regions}), a partir
...
%% son incompletos
Los fondos provenientes de electrones o jets mal identificados
...
el fondo de {\gjet}, que a pesar de ser despreciable en las SR, si la energía faltante
producida por la mala reconstrucción de los jets es elevada, puede ser relevante
...
\begin{figure}[!htbp]
  ...
  \includegraphics[width=0.49\textwidth]{signal_contamination_crwl}
  \includegraphics[width=0.49\textwidth]{signal_contamination_crwh}

  \includegraphics[width=0.49\textwidth]{signal_contamination_crtl}
  \includegraphics[width=0.49\textwidth]{signal_contamination_crth}
...
Como se menciono anteriormente, en la definición de las regiones de control, algunos cortes fueron relajados, o directamente removidos
...
la correspondiente CR (W o T). La selección detallada puede verse en \cref{tab:bkg_vrs1}.
...
debido a que la baja cantidad de eventos similares a la señal implicaria
usar una muestra con una muy alta cantidad de eventos.
%% , solo eventos
%% muy raros de la simulacion son utiles y seria necesario una muestra MC con
%% una muy alta cantidad de eventos.

Por este motivo se diseñó una región de control dominada por eventos de
{\gjet} a la que se llamó {\CRQ}. Los detalles de la selección se pueden ver en
la \cref{tab:bkg_crs}. Básicamente es la misma selección que la SR pero requiriendo baja
energia faltante ($\met < 50 \gev$).

%% La contaminación de multijets
...
de las SR en el espacio de observables, pero deben ser \hl{ortogonales} a las mismas.
...
El número de eventos de electrones reconstruidos como fotones en la region de
senal ($N_{e\to\gam}^{\mathrm{SR}}$) se obtiene multiplicando esta fracción (\feg)
...
de que resulta el límite mas desfavorable.
...
\hl{Agregar definición de la control sample y número finales en las SR}
...
Para respaldar la estrategia de \hl{derivar el modelo de aislamiento} de los fotones
...
eventos en las SR, lo que implicaria utilizar los datos en la region qie se
utiliza para la busqueda, se parametrizó este número como función de \met, como
...
\hl{Decir algo de las incertezas}
