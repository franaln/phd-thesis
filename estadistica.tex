...
tratamiento estadístico de los datos. Está enfocado en la búsqueda de nueva
...
entre hipótesis alternativas. %% Cuando la hipótesis a contrastar involucra el
%%valor de
...
\end{equation}
%
y la mejor región critica consiste en los $\bm{x}$ que satisface $t(\bm{x}) > c_\alpha$
...
En el límite asintotico ($N\to\infty$) el teorema de Wilk\cite{WilkTheo} muestra
...
0.05$. \hl{Si se considera el caso de $\mu=0$, se espera que por construcción el
...
\sim 0.05$) el {\cls} \hl{da un valor del orden de dos veces el obtenido utilizando
el {\clsb}}.
...
En este caso el estadístico $-2\ln\lambda$ sigue una distribución $\chi^2$ \hl{no central}.
...
\end{equation}
%
donde $\hat{\mu}$ sigue una distribución normal con media \hl{$\mu'$} y desviación
...
%% número de eventos de fondo tenga una incerteza sistemática
