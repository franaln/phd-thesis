...
A continuación se describen algunas distribuciones importantes que son necesarias
para ...

\subsubsection{Distribucion Gaussiana}
...
de la pdf Gaussiana es cuando $\mu=0$ y $\sigma=1$, la cual es denominada Gaussiana Estándar.
%% $\phi(x) = \frac{1}{\sqrt{2\pi}} \exp(-x^2/2)$.

La importancia de la distribución Gaussiana proviene del Teorema Central del Limite.
...
\subsubsection{Distribucion $\chi^2$}
...
\end{equation}
%
donde $n$ es llamado el numero de grados de libertad.
...
hipótesis nula es la hipótesis de que sólo los procesos del SM contribuyen a los
...
información de discriminación entre $H_0$ y $H_1$ en un sólo número.
...
%% generar lo que se denomina \emph{pseudo-experimento}. El procedimiento
...
intensidad de la señal de forma tal que la hipótesis de sólo-fondo corresponde a
...
En el limite asintotico ($N\to\infty$) el teorema de Wilk[] muestra
que $-2 \ln \lambda(\mu)$ sigue una distribucion $\chi^2$ con un grado de libertad
\note{chequear}. Esta propiedad sera util para aproximar la pdf del estaditico
...
\end{equation}
%
donde $p_b$ es el valor del mismo test bajo la hipótesis de sólo-fondo,
...
aproximaciones son formalmente validas en ese límite, las comparaciones con
Monte Carlo, muestran que el uso de las mismas son razonablemente precisas para
muestras de datos mucho \hl{mas chicas}, permitiendo su uso en la mayoría de los
