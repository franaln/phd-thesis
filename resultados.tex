...
%%     Sherlock Holmes, \emph{A Study in Scarlet}.
%%   \end{flushright}
...
\begin{equation}
  q_{\mu_s} = -2 \ln \left( \frac{L(\mu_s, \doublehat{\btheta})}{L(\hat{\mu}_s,\hat{\btheta})} \right)
\end{equation}
%
...
Los puntos de la derecha de la \cref{fig:fit_unc_nuisance}, después de la línea
punteada, representan los parámetros de normalización de los distintos fondos. Se
puede ver que todos los valores centrales están contenidos dentro de una desviación estándar.
...
\begin{figure}[!htbp]
  ...
  \includegraphics[width=0.45\textwidth]{plot_sr_l_met_et_after} \hspace{1cm}
  \includegraphics[width=0.45\textwidth]{plot_sr_h_met_et_after}
...
Puede observarse que los límites obtenidos, incluida la contribución de esta tesis,
...
distancia de $\unit[3.3]{cm}$ del tubo del haz.
...
\begin{figure}[!htb]
  ...
  \includegraphics[width=0.7\textwidth]{xs_run1grid_gg_8TeV_13TeV}
...
\begin{figure}[!h]
  ...
  \includegraphics[width=0.45\textwidth]{grid_updated}\hspace{1cm}
  \includegraphics[width=0.45\textwidth]{discovery_combined_13tev}

  \caption{Izquierda: Conjunto de puntos (\emph{grid}) de señal simulados para el análisis
    de \emph{Run} 2. Los cuadrados azules representan los puntos de señal
    generados con distintos valores de $m_{\gluino}$ y $m_{\ninoone}$.
    Derecha: Significancia de descubrimiento esperada para el análisis de datos de 13 \tev,
...
\begin{table}[!h]
  ...
  \begin{tabularx}{0.45\textwidth}{LRR}
    \hline
    & \SRL & \SRH \\
    \hline
    \nphotons & $=1$ & $=1$ \\
...
En la \cref{fig:13tev_grid} (derecha) se encuentra la significancia esperada de descubrimiento,
